...
import weight_sharing as ws

rozdeleni = {}
count = 0
for i in range(10):
    rozdeleni[i] = (count, count+35)
    count += 35
for i in range(10, 20):
    rozdeleni[i] = (count, count+30)
    count += 30
for i in range(20, 30):
    rozdeleni[i] = (count, count+25)
    count += 25
for i in range(30, 40):
    rozdeleni[i] = (count, count+20)
    count += 20
for i in range(40, 50):
    rozdeleni[i] = (count, count+15)
    count += 15
for i in range(50, 60):
    rozdeleni[i] = (count, count+10)
    count += 10
for i in range(60, 70):
    rozdeleni[i] = (count, count+8)
    count += 8
for i in range(70, 80):
    rozdeleni[i] = (count, count+5)
    count += 5
for i in range(80, 100):
    rozdeleni[i] = (count, count+3)
    count += 3
...
def first_level_combinations() -> List[Tuple[int, ...]]:
    all_combs = []
    for NR_OF_INDIV_WEIGHTS in range(1, 6):
        weight_ls = list(range(1, NR_OF_INDIV_WEIGHTS+1))
        combins = itertools.product(weight_ls, repeat=6)
...
def second_level_combinations(weight_ls: List[int], combins: List[Tuple[int, ...]]) -> List[Tuple[int, ...]]:
    ones_plus_minus = list(itertools.product([1, -1], repeat=6))
    plus_minus_combs = []
...
#     nr_combins_per_job = int(math.ceil(len(combs) / NR_OF_JOBS))
...
#         print(20*'-' + '\n\n')
    ...
    np.savez(f'data/combs_iters{ITERATIONS}.npz', combs=combinations)
